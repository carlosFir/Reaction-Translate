...
def train_step(model, inp, targ, optimizer, pad, teacher, step, length):
    targ_inp = targ[:, :-1]
    targ_real = targ[:, 1:]
    ...
    # teacher forcing uses 'targ' as the input to the decoder
    # non-teacher forcing uses 'prediction' as the input to the decoder, lasting until the end of the sequence
    
    # token level with 2 pass
    # first pass
    decoder_input = targ_inp
    e, c, d = create_mask(inp, decoder_input, pad)
    logits, _ = model(inp, decoder_input, e, c, d)
    logits_sft = torch.softmax(logits, dim=-1)
    logits_ids = torch.argmax(logits_sft, dim=-1)
    
    # pdb.set_trace()
    # mix
    replace_threshold = 1 - (1 / length) * (step + 1) # threshold schedule
    # print(replace_threshold)
    for index in range(logits.shape[1]):
        if torch.rand(1) < replace_threshold:
            # print('replace')
            logits_ids[:, index] = targ_inp[:, index]

    # second pass
    e, c, d = create_mask(inp, logits_ids, pad)
    prediction, _ = model(inp, logits_ids, e, c, d)

    # mix
    '''decoder_self = torch.tensor([[0] * batch_size]).transpose(0, 1).to(inp.device)
    for t in range(0, targ_real.shape[1]):
        
        threshold = 1 - 0.01 * t # linear
        # print(threshold)
        p = 1 if torch.rand(1) < threshold else 0s

        if p:
            decoder_input = targ_real
        else:
            decoder_input = decoder_self

        e, c, d = create_mask(inp, decoder_input, pad)
        prediction, _ = model(inp, decoder_input, e, c, d)
        prediction_ids = torch.argmax(prediction[:, -1, :], dim=-1).unsqueeze(1)
        decoder_self = torch.cat([decoder_self, prediction_ids], dim=1)'''

    # non tf
    '''decoder_input = torch.tensor([[0] * batch_size]).transpose(0, 1).to(inp.device)
    for t in range(0, targ_real.shape[1]):
        # print(decoder_input.shape)
        enc_padding_mask, combined_mask, dec_padding_mask = create_mask(inp, decoder_input, pad)
        prediction, _ = model(inp, decoder_input, enc_padding_mask, combined_mask, dec_padding_mask)
        # print(prediction.shape)
        prediction_ids = torch.argmax(prediction[:, -1, :], dim=-1).unsqueeze(1)

        decoder_input = torch.cat([decoder_input, prediction_ids], dim=1)'''
...
def validate_step(model, inp, targ, pad):
    # in validatiom ,there should be no targ as decoder_input.
    # instead, greedy decoder will be used first.
    # after that, beam search and sampling could be used to improve the generative ability.
    targ_inp = targ[:, :-1]
    targ_real = targ[:, 1:]
    batch_size = inp.shape[0]
    ...
    model.eval()
    with torch.no_grad():
        decoder_input = torch.tensor([[0] * batch_size]).transpose(0, 1).contiguous().to(inp.device)
        for t in range(targ_inp.shape[1]):
            e, c, d = create_mask(inp, decoder_input, pad)
            prediction, _ = model(inp, decoder_input, e, c, d)
            prediction_p = torch.softmax(prediction[:, -1, :], dim=-1)
            prediction_ids = torch.argmax(prediction_p, dim=-1).unsqueeze(1)
            decoder_input = torch.cat([decoder_input, prediction_ids], dim=1)
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=length) as t:
            ...
            for step, (inp, targ) in enumerate(train_dataloader):
                ...
                '''t.set_description(desc="Epoch {} Train".format(epoch), refresh=True)
        ...
        if epoch == epochs: 
            with tqdm(total=len(val_dataloader)) as t:
                for val_step, (inp, targ) in enumerate(val_dataloader):
                    ...
                    '''t.set_description(desc="Epoch {} Val".format(epoch), refresh=True)
                    t.set_postfix(steps=val_step, loss=val_loss_sum / (val_step + 1), acc=val_acc_sum / (val_step + 1))
                    t.update(1)'''
